...
describe('AuthService - Unit test', () => {
    ...
    beforeEach(() => {
         ...
         */
        authService = new AuthService_1.AuthService(mockUserRepository);
    ...
    describe('register()', () => {
        it('deberia registar a un nuevo usuario de manera exitosa y retornar el jwt', async () => {
            ...
            const expectedUser = new User_1.User(expect.any(String), validRegisterDto.email, hashedPassword, validRegisterDto.name);
            ...
            */
            expect(bcryptjs_1.default.hash).toHaveBeenCalledWith(validRegisterDto.password, 10);
             ...
             */
            expect(jsonwebtoken_1.default.sign).toHaveBeenCalledWith(expect.objectContaining({ email: validRegisterDto.email }), 'TU_SECRETO_JWT', { expiresIn: '1h' });
        });
        ...
        it('deberia generar un error cuando usuario ya esta registrado con ese email', async () => {
            ...
            // nota: tambien reutilizamos codigo de beforeEach
            const existingUser = new User_1.User('123', validRegisterDto.email, 'old password', 'old name');
